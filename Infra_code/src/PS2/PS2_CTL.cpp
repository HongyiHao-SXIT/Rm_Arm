...
void PS2_CTL::init(void)
{
  ...
  mode_status = PS2_SINGLE_SERVO_MODE;  // 默认单舵机模式
  memset(&last_keyvalue, 0, sizeof(last_keyvalue));  // 初始化，确保MOD边沿检测正确
}
...
void PS2_CTL::receive_msg(void)
{
  ...
  while (Serial_PS2.available()>0) {
    ...
    if(rec_flag)
    {
      ...
      // MOD键(recbuff[4] bit4)，切换逻辑在get_result中处理
      keyvalue.bit_mod = (recbuff[4] >> 4) & 1;
      keyvalue.mode = mode_status;
...
int PS2_CTL::get_result(LeArm_t* robot,Led_t* led,Buzzer_t* buzzer)
{
  if(rec_flag){
    rec_flag = false;
    // MOD键(recbuff[4] bit4)按下时切换模式
    if(keyvalue.bit_mod == 1 && last_keyvalue.bit_mod == 0)
    {
      mode_status = (mode_status == PS2_SINGLE_SERVO_MODE) ? PS2_COORDINATE_MODE : PS2_SINGLE_SERVO_MODE;
      keyvalue.mode = mode_status;
      buzzer->blink(1500, 100, 100, 1);  // 切换模式时蜂鸣器提示
    }
