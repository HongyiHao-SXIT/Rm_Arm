...
int PS2_CTL::get_result(LeArm_t* robot,Led_t* led,Buzzer_t* buzzer,Motor* motors)
{
  static int8_t motors_speed[4] = {0};
  if(rec_flag){
    rec_flag = false;
    if(keyvalue.mode == PS2_COORDINATE_MODE)
    {
      ...
      /* 手柄的x轴方向往右，y轴方向往下 */
      int x = map((float)((uint32_t)keyvalue.left_joystick_x), 0.0f, 255.0f, -100.0f, 100.0f); // 左右平移
      int y = -map((float)((uint32_t)keyvalue.left_joystick_y), 0.0f, 255.0f, -100.0f, 100.0f); // 前后运动
      int rx = map((float)((uint32_t)keyvalue.right_joystick_x), 0.0f, 255.0f, -100.0f, 100.0f); // 旋转
