...
void PwmServo_t::init(void)
{
  ...
  for (int i = 0; i < SERVO_NUM; ++i) {
    ...
    // Serial.print(pwm_servos[i].actual_pulsewidth);
    // Serial.print(" ");
  }
...
int  PwmServo_t::read_duty(uint16_t servo_index)
{
  ...
  return pwm_servos[servo_index].duration;
}
...
//保存偏差（未完成）
int  PwmServo_t::save_offset(void)
{
  return 0;
}
...
{
    if(pwm_servos[servo_index].actual_pulsewidth != pwm_servos[servo_index].target_pulsewidth)
    {
      return false;
    }else{
      return true;
