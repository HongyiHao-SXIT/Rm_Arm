...
// int adcValue = analogRead(36);  // 读取GPIO 34的ADC值
//   Serial.println(adcValue);      // 打印ADC值到串口
//   delay(500);                   // 延时1秒
...
uint8_t Button_t::read(uint8_t id)
{
  ...
  {
    button_id = 1;
  }else if(200 < adcValue && adcValue < 350)
  {
    button_id = 2;
  }else if(100 < adcValue && adcValue < 200)
  {
    button_id = 3;
  }
