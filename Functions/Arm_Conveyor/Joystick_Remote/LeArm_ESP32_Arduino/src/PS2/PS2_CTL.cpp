...
void PS2_CTL::PS2_Task(LeArm_t* robot,Led_t* led,Buzzer_t* buzzer,CONVEYOR_BELT* belt)
{
  ...
  receive_msg();
  get_result(robot,led,buzzer,belt);
  delay(20);
}
...
int PS2_CTL::get_result(LeArm_t* robot,Led_t* led,Buzzer_t* buzzer,CONVEYOR_BELT* belt)
{
  if(rec_flag){
    rec_flag = false;
    if(keyvalue.mode == PS2_COORDINATE_MODE)
    {
      ...
      int speed = map((float)((uint32_t)keyvalue.left_joystick_x), 0.0f, 255.0f, 100.0f, -100.0f);
